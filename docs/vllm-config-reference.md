# vLLM Configuration Reference (Latest / v0.20.x)

> Exhaustive reference of every configuration option, API parameter, engine argument, and knob in vLLM.
> Source: https://docs.vllm.ai/en/latest/ — Retrieved 2026-05-01.

---

## Table of Contents

1. [Engine Arguments (EngineArgs)](#1-engine-arguments-engineargs)
2. [Server / Frontend Arguments (AsyncEngineArgs)](#2-server--frontend-arguments)
3. [Sampling Parameters (SamplingParams)](#3-sampling-parameters-samplingparams)
4. [API Endpoints & Parameters](#4-api-endpoints--parameters)
5. [Quantization Methods](#5-quantization-methods)
6. [Model Configuration](#6-model-configuration)
7. [KV Cache & Memory](#7-kv-cache--memory)
8. [Parallelism (TP / PP / DP / EP / DCP / PCP)](#8-parallelism)
9. [Scheduling & Execution](#9-scheduling--execution)
10. [Speculative Decoding](#10-speculative-decoding)
11. [LoRA / Adapter Support](#11-lora--adapter-support)
12. [Structured Output / Guided Decoding](#12-structured-output--guided-decoding)
13. [Multimodal Configuration](#13-multimodal-configuration)
14. [Mamba / SSM Configuration](#14-mamba--ssm-configuration)
15. [Observability (Metrics, Tracing, Profiling)](#15-observability)
16. [Compilation & CUDA Graphs](#16-compilation--cuda-graphs)
17. [Offloading (CPU / UVA / Prefetch)](#17-offloading)
18. [Weight Transfer & KV Transfer (Disaggregated Serving)](#18-weight-transfer--kv-transfer)
19. [Attention Configuration](#19-attention-configuration)
20. [Kernel Configuration](#20-kernel-configuration)
21. [Environment Variables](#21-environment-variables)

---

## 1. Engine Arguments (EngineArgs)
...
### LoadConfig (Model Weight Loading)
...
### Chat Template & Frontend Options
...
| `--exclude-tools-when-tool-choice-none` | bool | `False` | |
...
## 3. Sampling Parameters (SamplingParams)
...
| `top_k` | int | None | Top-k sampling. Filter to top-k tokens. (vLLM extra, not in OpenAI) |
...
### Completions API (`/v1/completions`) — Standard OpenAI Params
...
### Chat Completions API (`/v1/chat/completions`) — Standard OpenAI Params
...
### Embeddings API (`/v1/embeddings`) — Standard Params
...
See also [Section 1 — Engine Arguments](#1-engine-arguments-engineargs) for the full list of `ModelConfig` arguments.
...
## 12. Structured Output / Guided Decoding
...
### ProfilerConfig (via `--profiler-config`)
...
## 18. Weight Transfer & KV Transfer

### WeightTransferConfig (via `--weight-transfer-config`)

Configuration for weight transfer during RL training.

### KVTransferConfig (via `--kv-transfer-config`)

Configuration for distributed KV cache transfer (disaggregated prefill/decode serving).

### KVEventsConfig (via `--kv-events-config`)

Configuration for event publishing.

### ECTransferConfig (via `--ec-transfer-config`)

Configuration for distributed EC cache transfer.

---

## 19. Attention Configuration
...
Key environment variables for runtime behavior:
...
You can pass all CLI arguments via a YAML config file:
...
Usage: `vllm serve --config config.yaml`

Priority: `command line > config file > defaults`

---

*This reference was compiled from the official vLLM documentation at https://docs.vllm.ai/en/latest/ on 2026-05-01, covering the latest developer preview (v0.20.x+).*

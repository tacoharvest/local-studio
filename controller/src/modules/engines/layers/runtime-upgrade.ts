...
const runCommandUpgrade = (command: string, args: string[]): RuntimeUpgradeResult => {
  ...
  return { success, version: null, output: result.stdout || null, error: success ? null : result.stderr || "Upgrade command failed", used_command: `${command} ${args.join(" ")}`.trim() };
};
...
export const upgradeSglangRuntime = async (config: Config, options: RuntimeUpgradeOptions = {}): Promise<RuntimeUpgradeResult> => {
  ...
  const args = useUv ? ["pip", "install", "--python", python, "--upgrade", "sglang"] : ["-m", "pip", "install", "--upgrade", "sglang"];
  ...
  if (commandResult.status !== 0) {
    return { success: false, version: runtime.version, output: commandResult.stdout || null, error: commandResult.stderr || "Failed to upgrade SGLang", used_command: useUv ? `uv ${args.join(" ")}` : `${python} ${args.join(" ")}` };
  }
  return { success: runtime.installed, version: runtime.version, output: commandResult.stdout || null, error: runtime.installed ? null : "Version check failed after upgrade", used_command: useUv ? `uv ${args.join(" ")}` : `${python} ${args.join(" ")}` };
};

export const upgradeLlamacppRuntime = async (config: Config, options: RuntimeUpgradeOptions): Promise<RuntimeUpgradeResult> => {
  const command = resolveCommand(options.command, LLAMACPP_UPGRADE_ENV);
  if (!command) return { success: false, version: null, output: null, error: "No llama.cpp upgrade command configured. Set VLLM_STUDIO_LLAMACPP_UPGRADE_CMD.", used_command: null };
  const parsedArguments = parseCommandInput(options.args);
...
export const runPlatformUpgrade = (platform: "cuda" | "rocm", options: RuntimeUpgradeOptions): RuntimeUpgradeResult => {
  const envKey = platform === "cuda" ? CUDA_UPGRADE_ENV : ROCM_UPGRADE_ENV;
  const command = resolveCommand(options.command, envKey);
  if (!command) return { success: false, version: null, output: null, error: `No ${platform.toUpperCase()} upgrade command configured. Set ${envKey}.`, used_command: null };
  const parsedArguments = parseCommandInput(options.args);
  const result = runCommandUpgrade(command, parsedArguments ?? []);
